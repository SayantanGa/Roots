...
export function GetInput(props) {
    ...
    return (
        <div className="form__group">
            <input type={props.type} className="form__input" placeholder={props.phtext || props.name } id={'form-login__' + props.type} pattern={props.patternRequired ? '[0-9]{10}' : null} value={props.value} onChange={props.onChangeHandler} {...props.attributes} required />
...
export function ContinueWithSocial({Alert, onLogin}) {
    ...
    const handleSubmit = (text) => {
        Alert(`${text} Authentication Successful!`);
        onLogin(true);
        setTimeout(() => {
            navigate('/');
        }, 1000);
